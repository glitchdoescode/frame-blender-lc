...
def check_node(node, father_node, frame_relation, reverse_order=False):
    ...
    try:
        ...
        return 1  # Assume valid if no JSON file exists (custom frame)
    ...
    if father_node.name == f"[{frame_relations[frame_relation][1 if not reverse_order else 0]}]" and not fathers or father_node.name in fathers:
...
def check_hierarchy(node, frame_relation, reverse_order=False):
    ...
    for son_node in node.next.values():
        if not check_node(son_node, node, frame_relation, reverse_order) or not check_hierarchy(son_node, frame_relation, reverse_order):
            return False
    return True
